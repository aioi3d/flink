...
MUST：LTRの横向き1ページページング、縦横回転、ピンチ拡大、手動の前／次ページ、現在ページ数表示、ページ数表示をタップして開くダイアログによるページ番号指定に対応する。右から左へのスワイプで次ページ、左から右へのスワイプで前ページへ移動する。PDF内部のページ順を採用し、左開き固定とする。初期表示はページ全体が収まる倍率。

MUST：瞬き、前／次、ページ番号ダイアログ、スワイプ完了による手動移動では、拡大率に関係なく移動先をページ全体表示に戻す。ページング遷移中は途中でfitを強制せず、確定したページにだけ適用する。最後のページで次へを実行しても折り返さず、先頭で前へを実行しても範囲外へ進まない。

MUST NOT：縦方向の連続スクロール、見開き、右開きは実装しない。
...
| 見開き等の表示モード | PDF表示設定を独立した設定オブジェクトにする。 | 見開き・縦方向の連続スクロール・右開きの実装。 |
