...
| P1-09 Native signature | CI_VERIFIED | Run 7 recorded the historical runtime 1.0.5 development signature `1983454e6ddbcd9ef947f7af56ea2441655f5deba15d23ef93fd4fe62e9a544f`. Runtime 1.0.11 changes native inputs while retaining API 2, so its development signature is intentionally unrecorded until CI builds it. No generated metadata, Git state, or absolute manifest path is hashed. Production remains intentionally unrecorded. |
...
| P2-09 Context and runtime metadata | IMPLEMENTED_UNVERIFIED | Process-local ContextBroker rejects stale/suspended reader, generation, epoch, and sample-time combinations. Run 7 verified and published embedded 1.0.5 metadata; replacement 1.0.11 / API 2 metadata is pending, and broker behavior still requires devices. |
| P2-10 Native smoke screen | IMPLEMENTED_UNVERIFIED | Minimal iOS screen exercises init/scan/import/cancel/rename/delete/thumbnail, PDF open/navigation, capabilities/permission/start/stop/reset/drain, numeric coefficients, debug face, and runtime metadata. Runtime 1.0.11 auto-requests library thumbnails, replaces the page text field and `移動` button with a page-count dialog, and retains private automatic fitting only for explicit navigation and layout/lifecycle events. |
| P2-11 CI and first IPA | CI_VERIFIED | Workflow run 7 for `dev-runtime-v1.0.5` / source `c23ff74adfc37c439a98652106e461dca222a4eb` completed Linux preflight, pinned macOS build, app/IPA inspection, checksum verification, and GitHub Release publication. It remains valid historical evidence; runtime 1.0.11 now needs a replacement build. Same-tag rerun behavior remains tracked separately by TC-D08. |
...
|---|---|
| Native API | 2 |
| Native runtime version | 1.0.11 (library-thumbnails-and-continuous-reader build pending) |
| Algorithm | SHA-256, signature input schema 1 |
| Recorded installed signature | development: unrecorded for 1.0.11 / API 2; production: unrecorded. The 1.0.5 value remains in its Release metadata. |
...
| Installed build metadata | Pending for 1.0.11 / API 2; stale 1.0.5 metadata was removed from the canonical comparison path and remains in `releases/assets/1.0.5`. |

Until 1.0.11 metadata is recorded, `native:check -- --profile development` must return `unresolved` with no mismatches, while `--require-resolved` must fail closed. Production remains unrecorded and must be checked explicitly only when a production runtime is prepared.
...
The 8-file / 114-test aggregate above is the retained Phase 1 baseline. The 11-file / 156-test result remains the retained runtime 1.0.10 aggregate after Release input integration, fail-closed profile inference, promised coordinator-resource validation, identifier-or-inode identity proof, native-view readiness coverage, safe path diagnostics, rooted mutation preflight, and removal of the manual page-fit API/UI. The current runtime 1.0.11 aggregate passed with 11 files / 157 tests, production/development configuration checks, the expected unresolved API 2 / runtime 1.0.11 native compatibility check with zero mismatches, and workflow-policy validation. Workflow runs 6 and 7 remain historical macOS build/package/publication evidence; runtime 1.0.11 still requires CI and both target devices.
...
2. Commit and push the 1.0.11 library-thumbnail and continuous-reader changes, create the exact source tag `dev-runtime-v1.0.11`, then manually run `build-ios-ipa.yml` with profile `development`. A development-tag push alone does not start a native build. Do not reuse or move an existing development tag.
3. Verify and record the 1.0.11 / API 2 Release metadata, checksum manifest, and Pod lock before requiring a resolved native check. Never move an existing tag to new source.
4. Re-sign/install the 1.0.11 IPA and confirm automatic library thumbnails, vertical continuous scrolling, the page-count dialog, retained previous/next controls, passive-scroll zoom retention, and suppression of external actions on partially visible pages. Confirm that manual `ページ全体` remains absent while opening, explicit navigation, and rotating PDFs retain automatic fitting. Include device model / OS if available.
