...
version: 1.0.12-phase2-horizontal-single-page-paging
updated_at: 2026-09-10
scope: Phase 1 and Phase 2
status: phase_2_runtime_1_0_12_horizontal_single_page_paging_pending_ci_and_device_regression
---

# Flink implementation status

This ledger separates implemented source, local verification, CI verification, and device verification. Runtime `1.0.6` retained the `isEqual(_:)` physical-identity repair, but device testing still returned `E_PATH_OUTSIDE_LIBRARY` during rename. Runtime `1.0.7` uses Foundation's promised-item resource values and concurrent source, parent, and library-root access intents. Runtime `1.0.8` adds a fixed, path-free diagnostic ID for rename/delete path-boundary rejections. Runtime `1.0.9` rebuilds every preflight operation URL from validated relative components beneath the current app-owned library root; the user then confirmed rename succeeds. Runtime `1.0.10` removes the manual `ページ全体` bridge and UI, while retaining automatic fit for opening, explicit navigation, and layout changes; this intentional API removal raised the native API to 2. Runtime `1.0.11` introduced automatic library thumbnails and the page-count dialog, and its vertical continuous-reader experiment remains historical. Runtime `1.0.12` replaces that display mode with LTR horizontal single-page PageViewController paging: right→left swipes move next, left→right swipes move previous, and a completed swipe applies fit. Previous/next, the page-count dialog, automatic thumbnails, and visible-page external-action suppression remain. Its IPA and device regression are pending, so the Phase 2 exit gate is not claimed.
...
| P1-06 Typed native API contract | LOCAL_VERIFIED | TypeScript contract, validation, mocks, and explicit unavailable-native behavior passed the aggregate local checks. The current contract version is API 2. |
...
| P1-09 Native signature | CI_VERIFIED | Run 7 recorded the historical runtime 1.0.5 development signature `1983454e6ddbcd9ef947f7af56ea2441655f5deba15d23ef93fd4fe62e9a544f`. Runtime 1.0.12 changes native inputs while retaining API 2, so its development signature is intentionally unrecorded until CI builds it. No generated metadata, Git state, or absolute manifest path is hashed. Production remains intentionally unrecorded. |
...
| P2-04 Mutations and presenters | IMPLEMENTED_UNVERIFIED | Runtime 1.0.6 still rejected rename with `E_PATH_OUTSIDE_LIBRARY`. Runtime 1.0.7 uses `promisedItemResourceValues(forKeys:)` inside `.forMoving` / `.forDeleting` accessors as Foundation requires and coordinates source, existing parent, and library root as concurrent intents. Runtime 1.0.8 adds fixed, path-free diagnostics. Runtime 1.0.9 rebuilds the resolve, rename, and delete preflight URLs from validated indexed relative components below the current root, then retains symbolic-link, fingerprint, coordinator, and physical-identity validation; the user confirmed rename succeeds. The runtime 1.0.12 replacement build remains pending. |
| P2-05 PDF view | IMPLEMENTED_UNVERIFIED | Runtime 1.0.5 device observation confirmed PDF content display, previous/next navigation, and page-number jumps. Runtime 1.0.10 removes the manual current-page fit/zoom-reset button and native API. Runtime 1.0.11's vertical continuous display remains historical. Runtime 1.0.12 uses LTR horizontal single-page PageViewController paging: right→left swipes move next, left→right swipes move previous, and the completed swipe fits its destination. Prior/next remain, and a tappable page count opens the page-number dialog. Private automatic fitting remains for open, explicit navigation, completed swipes, layout changes, and lifecycle resume. Every visible page, including a transition neighbor, has external PDF actions disabled. Device confirmation of the new build and device identity/OS are still unrecorded. |
| P2-06 Thumbnails | IMPLEMENTED_UNVERIFIED | The library automatically requests its entry thumbnails serially; the native bounded queue provides cancellation/coalescing, page-0 PDFKit rendering, 512 px output, revision cache key, 32 MiB decoded/128 MiB disk limits, reader priority, memory warning, and thermal pause. Cache and pressure behavior requires devices. |
...
| P2-09 Context and runtime metadata | IMPLEMENTED_UNVERIFIED | Process-local ContextBroker rejects stale/suspended reader, generation, epoch, and sample-time combinations. Run 7 verified and published embedded 1.0.5 metadata; replacement 1.0.12 / API 2 metadata is pending, and broker behavior still requires devices. |
| P2-10 Native smoke screen | IMPLEMENTED_UNVERIFIED | Minimal iOS screen exercises init/scan/import/cancel/rename/delete/thumbnail, PDF open/navigation, capabilities/permission/start/stop/reset/drain, numeric coefficients, debug face, and runtime metadata. Runtime 1.0.12 auto-requests library thumbnails, replaces the page text field and `移動` button with a page-count dialog, and uses LTR horizontal one-page swipes with fit at transition completion while retaining private automatic fitting for explicit navigation and layout/lifecycle events. |
| P2-11 CI and first IPA | CI_VERIFIED | Workflow run 7 for `dev-runtime-v1.0.5` / source `c23ff74adfc37c439a98652106e461dca222a4eb` completed Linux preflight, pinned macOS build, app/IPA inspection, checksum verification, and GitHub Release publication. It remains valid historical evidence; runtime 1.0.12 now needs a replacement build. Same-tag rerun behavior remains tracked separately by TC-D08. |
...
|---|---|
| Native API | 2 |
| Native runtime version | 1.0.12 (horizontal-single-page-paging build pending) |
| Algorithm | SHA-256, signature input schema 1 |
| Recorded installed signature | development: unrecorded for 1.0.12 / API 2; production: unrecorded. The 1.0.5 value remains in its Release metadata. |
...
| Installed build metadata | Pending for 1.0.12 / API 2; stale 1.0.5 metadata was removed from the canonical comparison path and remains in `releases/assets/1.0.5`. |

Until 1.0.12 metadata is recorded, `native:check -- --profile development` must return `unresolved` with no mismatches, while `--require-resolved` must fail closed. Production remains unrecorded and must be checked explicitly only when a production runtime is prepared.
...
| 2026-09-10 | runtime 1.0.11 library thumbnails and continuous reader | Changed the library UI to request page-0 thumbnails automatically and serially, changed PDFKit to vertical continuous display, and replaced the editable page field plus `移動` button with a page-count dialog while retaining prior/next. Automatic fit remains for open, explicit navigation, layout, and lifecycle resume; passive scrolling does not reset scale or position. Visible PDF pages all have external actions disabled. `npm.cmd run verify:local` passed typecheck, lint, 11 files / 157 tests, production/development Expo configuration, the expected unresolved default native check for API 2 / runtime 1.0.11, and workflow policy. The explicit development native check also returned the expected unresolved result with zero mismatches. Native compile, IPA build, and device regression remain pending. |
| 2026-09-10 | runtime 1.0.12 horizontal single-page paging | Replaced runtime 1.0.11's vertical continuous display with LTR horizontal one-page PageViewController paging: right→left swipes move next, left→right swipes move previous, and a completed swipe applies fit. Automatic library thumbnails, prior/next, and the page-count dialog remain. Visible-page external-action suppression remains active for the adjacent page exposed during a PageViewController transition. `npm.cmd run verify:local` passed typecheck, lint, 11 files / 157 tests, production/development Expo configuration, the expected unresolved default native check for API 2 / runtime 1.0.12, and workflow policy. The explicit development native check also returned the expected unresolved result with zero mismatches. Native API remains 2; native compile, IPA build, and device regression remain pending. |
...
The 8-file / 114-test aggregate above is the retained Phase 1 baseline. The 11-file / 156-test result remains the retained runtime 1.0.10 aggregate after Release input integration, fail-closed profile inference, promised coordinator-resource validation, identifier-or-inode identity proof, native-view readiness coverage, safe path diagnostics, rooted mutation preflight, and removal of the manual page-fit API/UI. Runtime 1.0.11's 11-file / 157-test result remains historical evidence for its continuous-reader behavior. The current runtime 1.0.12 aggregate passed with 11 files / 157 tests, production/development configuration checks, the expected unresolved API 2 / runtime 1.0.12 native compatibility check with zero mismatches, and workflow-policy validation. Workflow runs 6 and 7 remain historical macOS build/package/publication evidence; runtime 1.0.12 still requires CI and both target devices.
...
2. Commit and push the 1.0.12 horizontal-single-page-paging changes, create the exact source tag `dev-runtime-v1.0.12`, then manually run `build-ios-ipa.yml` with profile `development`. A development-tag push alone does not start a native build. Do not reuse or move an existing development tag.
3. Verify and record the 1.0.12 / API 2 Release metadata, checksum manifest, and Pod lock before requiring a resolved native check. Never move an existing tag to new source.
4. Re-sign/install the 1.0.12 IPA and confirm automatic library thumbnails, LTR horizontal one-page paging (right→left next / left→right previous), the page-count dialog, retained previous/next controls, fit after a completed swipe, and suppression of external actions on a transition's partially visible neighboring page. Confirm that manual `ページ全体` remains absent while opening, explicit navigation, and rotating PDFs retain automatic fitting. Include device model / OS if available.
