...
status: phase_2_runtime_1_0_5_recorded_device_regression_pending
---

# Flink implementation status

This ledger separates implemented source, local verification, CI verification, and device verification. The `dev-runtime-v1.0.4` workflow completed the first Phase 2 development path end to end. Physical-device testing then confirmed import, direct Files placement, delete, and on-demand thumbnails, while exposing two blockers: rename returned `E_PATH_OUTSIDE_LIBRARY`, and the first reader open returned Expo's `ERR_VIEW_NOT_FOUND` before PDFKit ran. Runtime `1.0.5` repairs both paths; workflow run 7 compiled and published it, and its downloaded Pod lock, build metadata, and development signature are now recorded locally. Device regression results are still pending, so the Phase 2 exit gate is not claimed.
...
| P1-09 Native signature | CI_VERIFIED | Run 7 recorded the runtime 1.0.5 development signature `1983454e6ddbcd9ef947f7af56ea2441655f5deba15d23ef93fd4fe62e9a544f`; local calculation over 34 inputs reproduces it with no unresolved inputs. No generated metadata, Git state, or absolute manifest path is hashed. Production remains intentionally unrecorded. |
...
| P2-04 Mutations and presenters | IMPLEMENTED_UNVERIFIED | Device testing confirmed delete but found rename falsely rejecting an equivalent `NSFileCoordinator` accessor URL as outside the library. Runtime 1.0.5 proves the accessor's derived root is the same physical library, revalidates both coordinated source and destination, preserves component-level symlink rejection, and canonicalizes the post-rename lookup. The repaired IPA is built; device regression is pending. |
...
| P2-09 Context and runtime metadata | IMPLEMENTED_UNVERIFIED | Process-local ContextBroker rejects stale/suspended reader, generation, epoch, and sample-time combinations. Run 7 verified and published the embedded 1.0.5 metadata; broker behavior still requires devices. |
| P2-10 Native smoke screen | IMPLEMENTED_UNVERIFIED | Minimal iOS screen exercises init/scan/import/cancel/rename/delete/thumbnail, PDF open/navigation/fit, capabilities/permission/start/stop/reset/drain, numeric coefficients, debug face, and runtime metadata. Device testing confirmed the file operations listed above and exposed the two repaired blockers; the remaining smoke path now requires regression testing with runtime 1.0.5. |
| P2-11 CI and first IPA | CI_VERIFIED | Workflow run 7 for `dev-runtime-v1.0.5` / source `c23ff74adfc37c439a98652106e461dca222a4eb` completed Linux preflight, pinned macOS build, app/IPA inspection, checksum verification, and GitHub Release publication. Its Release metadata and Pod lock are recorded locally. Same-tag rerun behavior remains tracked separately by TC-D08. |
...
|---|---|
| Native API | 1 |
| Native runtime version | 1.0.5 (development build recorded) |
| Algorithm | SHA-256, signature input schema 1 |
| Recorded installed signature | development: `1983454e6ddbcd9ef947f7af56ea2441655f5deba15d23ef93fd4fe62e9a544f`; production: unrecorded. The 1.0.4 value remains in its Release metadata. |
...
| Installed build metadata | Runtime 1.0.5 run 7 metadata recorded at `config/installed-native-build-info.json`; tag `dev-runtime-v1.0.5`, source `c23ff74adfc37c439a98652106e461dca222a4eb`, build 7. |

`native:check -- --require-resolved` now infers the installed development profile and must return `compatible`. An explicit `--profile development` check must produce the same signature. Production remains unrecorded and must be checked explicitly only when a production runtime is prepared.
...
The 8-file / 114-test aggregate above is the retained Phase 1 baseline. The current aggregate is 11 files / 151 tests after Release input integration, fail-closed profile inference, coordinator-alias validation, and native-view readiness coverage. Workflow run 6 remains historical evidence for 1.0.4; workflow run 7 supplies current macOS build/package/publication evidence for repaired runtime 1.0.5. Both target devices still require regression testing.
...
2. Optionally place `Flink-dev-runtime-1.0.5-unsigned.ipa` beside the three downloaded evidence files and compare its SHA-256 with `5d4a79f3d12c7c899364694f3f50ae8061968fb1a8b9f626f6952d29ebcf62d6`; the workflow already checked it, but the local handoff currently contains no IPA bytes to rehash.
3. Re-sign/install the 1.0.5 IPA and repeat rename plus PDF open/previous/next/number jump/fit separately on both target devices. Include model, OS, and exact error code if any failure remains.
4. Keep the 1.0.4 Release assets as historical evidence. A same-tag rerun for TC-D08 is optional and separate; never move an existing tag to new source.
5. Do not create a production `v*` tag yet. Reserve it until the repaired development device gate succeeds. Keep Apple certificates, provisioning profiles, account passwords, and App Store Connect keys out of this workflow.
