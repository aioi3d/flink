...
version: 1.0.6-phase2-rename-repair
updated_at: 2026-09-10
scope: Phase 1 and Phase 2
status: phase_2_runtime_1_0_6_repair_pending_ci_and_device_regression
---

# Flink implementation status

This ledger separates implemented source, local verification, CI verification, and device verification. Runtime `1.0.5` and workflow run 7 repaired the PDF-view mount race: the next device session displayed PDF content and passed previous/next plus page-number navigation. Rename still returned `E_PATH_OUTSIDE_LIBRARY`. The remaining cause was the 1.0.5 physical-root helper hashing the archive representation of opaque file identifiers instead of using Foundation's required `isEqual(_:)` comparison. Runtime `1.0.6` corrects that comparison while retaining containment and symlink rejection. Its replacement IPA and device regression are pending, so the Phase 2 exit gate is not claimed.
...
| P1-09 Native signature | CI_VERIFIED | Run 7 recorded the historical runtime 1.0.5 development signature `1983454e6ddbcd9ef947f7af56ea2441655f5deba15d23ef93fd4fe62e9a544f`. Runtime 1.0.6 changes native inputs, so its development signature is intentionally unrecorded until CI builds it. No generated metadata, Git state, or absolute manifest path is hashed. Production remains intentionally unrecorded. |
...
| P2-04 Mutations and presenters | IMPLEMENTED_UNVERIFIED | Delete is observed working, but rename still failed in runtime 1.0.5. The remaining false rejection was isolated to physical-root comparison: `fileResourceIdentifier` is opaque and must be compared with `isEqual(_:)`, not serialized and hashed. Runtime 1.0.6 uses the documented equality contract while preserving coordinated source/destination validation, component-level symlink rejection, atomic no-replace rename, and canonical post-rename lookup. Replacement-IPA regression is pending. |
| P2-05 PDF view | IMPLEMENTED_UNVERIFIED | Runtime 1.0.5 device observation confirmed PDF content display, previous/next navigation, and page-number jumps. The button formerly labelled `全体表示` is a current-page fit/zoom reset, not an all-pages list; because each page initially opens fitted, pressing it before zooming can correctly show no change. Runtime 1.0.6 relabels it `ページ全体` and explains the behavior. Zoom-then-fit and device identity/OS are still unrecorded. |
...
| P2-09 Context and runtime metadata | IMPLEMENTED_UNVERIFIED | Process-local ContextBroker rejects stale/suspended reader, generation, epoch, and sample-time combinations. Run 7 verified and published embedded 1.0.5 metadata; replacement 1.0.6 metadata is pending, and broker behavior still requires devices. |
| P2-10 Native smoke screen | IMPLEMENTED_UNVERIFIED | Minimal iOS screen exercises init/scan/import/cancel/rename/delete/thumbnail, PDF open/navigation/fit, capabilities/permission/start/stop/reset/drain, numeric coefficients, debug face, and runtime metadata. Runtime 1.0.5 confirmed PDF open and navigation, while exposing the remaining rename comparator bug. The next smoke pass requires runtime 1.0.6. |
| P2-11 CI and first IPA | CI_VERIFIED | Workflow run 7 for `dev-runtime-v1.0.5` / source `c23ff74adfc37c439a98652106e461dca222a4eb` completed Linux preflight, pinned macOS build, app/IPA inspection, checksum verification, and GitHub Release publication. It remains valid historical evidence; runtime 1.0.6 now needs a replacement build for the remaining rename regression. Same-tag rerun behavior remains tracked separately by TC-D08. |
...
|---|---|
| Native API | 1 |
| Native runtime version | 1.0.6 (replacement build pending) |
| Algorithm | SHA-256, signature input schema 1 |
| Recorded installed signature | development: unrecorded for 1.0.6; production: unrecorded. The 1.0.5 value remains in its Release metadata. |
...
| Installed build metadata | Pending for 1.0.6; stale 1.0.5 metadata was removed from the canonical comparison path and remains in `releases/assets/1.0.5`. |

Until 1.0.6 metadata is recorded, `native:check -- --profile development` must return `unresolved` with no mismatches, while `--require-resolved` must fail closed. Production remains unrecorded and must be checked explicitly only when a production runtime is prepared.
...
The 8-file / 114-test aggregate above is the retained Phase 1 baseline. The current aggregate is 11 files / 151 tests after Release input integration, fail-closed profile inference, coordinator-alias validation, identifier-equality enforcement, and native-view readiness coverage. Workflow runs 6 and 7 remain historical macOS build/package/publication evidence; runtime 1.0.6 still requires CI and both target devices.
...
2. Commit and push the 1.0.6 repair, create the exact source tag `dev-runtime-v1.0.6`, then manually run `build-ios-ipa.yml` with profile `development`. A development-tag push alone does not start the workflow.
3. Verify and record the 1.0.6 Release metadata, checksum manifest, and Pod lock before requiring a resolved native check. Never move an existing tag to new source.
4. Re-sign/install the 1.0.6 IPA and repeat rename separately on both target devices. Test page fit by pinching to zoom, panning away, then pressing `ページ全体`; the same page should shrink back completely into the viewport. Include device model, OS, and exact error code for any failure.
